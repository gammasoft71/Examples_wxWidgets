  ...
  public:
    Frame() : wxFrame {nullptr, wxID_ANY, "KeyEvents"} {
      control->Bind(wxEVT_KEY_DOWN, [&](wxKeyEvent& event) {
        wxLogDebug(wxString::Format("KeyDown={KeyCode=%s, Modifiers=[%s]}", KeyCodeToString(event.GetKeyCode()), ModiiersToString(event.GetModifiers())));
        event.Skip();
      });
      ...
      control->Bind(wxEVT_KEY_UP, [&](wxKeyEvent& event) {
        wxLogDebug(wxString::Format("KeyUp={KeyCode=%s, Modifiers=[%s]}", KeyCodeToString(event.GetKeyCode()), ModiiersToString(event.GetModifiers())));
